...
def admin_panel():
    db = get_db()
    cursor = db.cursor()
    cursor.execute('SELECT id, nome_nascido, mae_nome, pai_nome FROM nascimentos ORDER BY id DESC')
    nascimentos = cursor.fetchall()
    cursor.execute('SELECT id, falecido_nome, declarante_nome FROM obitos ORDER BY id DESC')
    obitos = cursor.fetchall()
    cursor.execute('SELECT id, noivo1_nome, noivo2_nome FROM casamentos ORDER BY id DESC')
    casamentos = cursor.fetchall()
    cursor.close()
...
def detalhes_nascimento(id):
    db = get_db()
    cursor = db.cursor()
    cursor.execute('SELECT * FROM nascimentos WHERE id = %s', (id,))
...
def detalhes_obito(id):
    db = get_db()
    cursor = db.cursor()
    cursor.execute('SELECT * FROM obitos WHERE id = %s', (id,))
    registro = cursor.fetchone()
    cursor.close()
    if registro is None: abort(404)
...
def detalhes_casamento(id):
    db = get_db()
    cursor = db.cursor()
    cursor.execute('SELECT * FROM casamentos WHERE id = %s', (id,))
    registro = cursor.fetchone()
    cursor.close()
    if registro is None: abort(404)
